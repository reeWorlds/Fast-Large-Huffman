...
namespace HuffDecodeV3_21_1
{
	...
	uint32_t _n[SKIP_SIZE];
	uint32_t _shift[SKIP_SIZE];
	uint64_t _L21[SKIP_SIZE];
	...
	void precompute(uint8_t* huffTable, uint32_t huffTableLen, uint32_t dictSize)
	{
		...
		vector <uint32_t> L21_list;
		vector <uint32_t> L21_lens_list;

		for (int32_t firstBits = 0; firstBits < SKIP_SIZE; firstBits++)
		{
			L21_list.clear();
			L21_lens_list.clear();

			uint32_t blockCode = (firstBits << (32 - SKIP_BITS));
			int32_t remainingLength = SKIP_BITS;

			while (true)
			{
				int32_t l = t_minHuffLen;
				while (t_limit[l] <= blockCode)
				{
					l++;
				}
				...
				{
					uint32_t curCode = blockCode >> (32 - l);
					curCode += t_firstCodeDiff[l];
					L21_list.push_back(curCode);
					L21_lens_list.push_back(l);
			...
			else if (L21_list.size() == 1)
			{
				_n[firstBits] = 1;
				_shift[firstBits] = L21_lens_list[0];
				_L21[firstBits] = L21_list[0];
			}
			else
			{
				_n[firstBits] = 2;
				_shift[firstBits] = L21_lens_list[0] + L21_lens_list[1];
				_L21[firstBits] = L21_list[0] | (((uint64_t)L21_list[1]) << 32);
			}
		}
	}
		...
		uint32_t streamLen)
	{
		...
		int32_t skipShift = 64 - SKIP_BITS;
		...
		for (int32_t codeI = 0; codeI < cntCodes;)
		{
			for (int32_t i = 0; i < ITERS_PER_BITSTREAM; i++)
			{
				...
				bitStream <<= _shift[blockCode];
				*(uint64_t*)(codes + codeI) = _L21[blockCode];
				codeI += _n[blockCode];

				blockCode = bitStream >> 32;
