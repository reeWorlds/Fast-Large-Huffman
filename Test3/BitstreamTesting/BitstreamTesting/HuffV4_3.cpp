...
namespace HuffDecodeV4_3
{
	...
	uint32_t _n[LOOKUP_SIZE];
	uint32_t _shift[LOOKUP_SIZE];
	uint64_t _L21[LOOKUP_SIZE];
	...
	void precompute(uint8_t* huffTable, uint32_t huffTableLen, uint32_t dictSize)
	{
		...
		for (int32_t firstBits = 0; firstBits < LOOKUP_SIZE; firstBits++)
		{
			...
			uint32_t blockCode = (firstBits << (32 - LOOKUP_BITS));
			int32_t remainingLength = LOOKUP_BITS;
		...
		uint32_t streamLen)
	{
		...
		{
			uint32_t lookupBits = bitStream >> lookupShift;
			uint32_t n = _n[lookupBits];

			if (n > 0)
			{
				bitStreamRequiredShift += _shift[lookupBits];
				bitStream <<= _shift[lookupBits];
				*(uint64_t*)(codes + codeI) = _L21[lookupBits];
				codeI += n - 1;
			}
			else
			{
				uint32_t blockCode = bitStream >> 32;
				int32_t l = t_first[lookupBits];
